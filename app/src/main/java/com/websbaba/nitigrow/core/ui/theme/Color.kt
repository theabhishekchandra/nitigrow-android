...
private object Hex {
    ...
    // Brand Forward — cream hero surfaces + deep-green heading ink
    val Paper2BrandForward    = Color(0xFFF7EED7) // cream hero surface
    val Paper3BrandForward    = Color(0xFFF2E7CB)
    val BrandInkBrandForward  = Color(0xFF1F4D36) // deep-green heading ink
    val CardHoverBrandForward = Color(0xFFFAF4E4)

    // Espresso Premium — turmeric takes over as the primary CTA colour
    val BrandEspresso      = Turmeric             // #E8A94A
    val BrandHoverEspresso = Color(0xFFF5D78C)    // link gold
    val BrandSoftEspresso  = Color(0x24E8A94A)    // rgba(232,169,74,0.14)
    val BrandInkEspresso   = Color(0xFF4A2F0A)    // ink on turmeric CTA
    val BrandRingEspresso  = Color(0x3DE8A94A)    // rgba(232,169,74,0.24)
...
// Brand Forward (AppTheme.BRAND_FORWARD, light) — Soft Paper's palette with
// cream hero surfaces and deep-green heading ink. Only the deltas below differ
// from NitiGrowLightColors; everything else is inherited verbatim.
val BrandForwardColors = NitiGrowLightColors.copy(
    brandInk  = Hex.BrandInkBrandForward,
    paper2    = Hex.Paper2BrandForward,
    paper3    = Hex.Paper3BrandForward,
    cardHover = Hex.CardHoverBrandForward,
)

// Espresso Premium (AppTheme.ESPRESSO_PREMIUM, inherently dark) — the dark
// palette with turmeric promoted to the primary/CTA role. Surfaces, semantic
// colours, chat bubbles, sidebar and avatars stay as in NitiGrowDarkColors.
val EspressoPremiumColors = NitiGrowDarkColors.copy(
    brand      = Hex.BrandEspresso,
    brandHover = Hex.BrandHoverEspresso,
    brandSoft  = Hex.BrandSoftEspresso,
    brandInk   = Hex.BrandInkEspresso,
    brandRing  = Hex.BrandRingEspresso,
)
